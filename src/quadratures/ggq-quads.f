...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine zgetnearquad_ggq_guru_c(npatches,norders,
...
c
      ...
      integer, intent(in) :: npatches,norders(npatches),npts
      integer, intent(in) :: ixyzs(npatches+1),iptype(npatches)
      complex *16, intent(in) :: srccoefs_c(9,npts),srcvals_c(12,npts)
      real *8 :: eps
      integer, intent(in) :: ntarg,ipatch_id(ntarg)
      real *8, intent(in) :: uvs_targ(2,ntarg)
      complex *16, intent(in) :: targvals_c(ndtarg,ntarg)
      real *8, intent(in) :: dpars(ndd)
      complex *16, intent(in) :: zpars(ndz)
      integer, intent(in) :: nnz
      integer, intent(in) :: row_ptr(ntarg+1),col_ind(nnz),iquad(nnz+1)
      ...
      real *8, allocatable :: rads(:)
      complex *16, allocatable :: targ_near(:,:),targ_far(:,:)
      integer, allocatable :: iind_near(:),iind_far(:)
      real *8, allocatable :: umatr(:,:),vmatr(:,:),uvs(:,:),wts(:)
...
c
      ...
      complex *16, allocatable :: xyztarg2(:,:)
      integer irad
...
c
     ...
     1   row_ind,iper)
      ...
      allocate(cms_c(3,npatches),rads(npatches))


c        find chunk radisu and centroid again
c

      call get_centroid_rads_c(npatches,norders,ixyzs,iptype,npts,
     1     srccoefs_c,cms_c,rads,ifre)
c
c
c       get adaptive integration + bremer self quadrature parameters
c
...
C$OMP$PRIVATE(ntarg_f,ntarg_n,npols,norder,irad)
C$OMP$PRIVATE(uvs,umatr,vmatr,wts)
C$OMP$PRIVATE(epsp,rsc,tmp,tmp_c)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        ii = ixyzs(ipatch)
        call cross_prod3d_c(srcvals_c(4,ii),srcvals_c(7,ii),tmp_c)
        rsc = abs((tmp_c(1)**2 + tmp_c(2)**2 + tmp_c(3)**2)*wts(1)**2)
        do i=2,npols
          call cross_prod3d_c(srcvals_c(4,ii+i-1),
     1                         srcvals_c(7,ii+i-1),tmp_c)
          rr = abs((tmp_c(1)**2 + tmp_c(2)**2 + tmp_c(3)**2)*wts(i)**2)
          if(rr.lt.rsc) rsc = rr
        enddo
        epsp = eps_adap*rsc**0.25d0
        ...
        do i=col_ptr(ipatch),col_ptr(ipatch+1)-1
          ...
          if(ipatch.ne.jpatch) then
            do iiif=1,ndtarg
               xyztarg2(iiif,ii2) = targvals_c(iiif,jtarg)
            enddo
            ii2 = ii2 + 1
          endif
        enddo
...
c
       rr = rfac0*rads(ipatch)
       call get_near_far_split_pt_c(ndtarg,ntarg2,xyztarg2,rr,
     1        cms_c(1,ipatch),ntarg_f,ntarg_n,targ_far,targ_near,
     2        iind_far,iind_near,ifre)
...
c
        ...
        if(iptype(ipatch).eq.1) 
     1      call ctriaints_c(epsp,istrat,intype,ntest0,norder,npols,
     1      srccoefs_c(1,istart),ndtarg,ntarg_n,targ_near,ifp,xyztarg2,
     2      itargptr,ntarg_n,norder,npols,fker,ndd,dpars,ndz,zpars,ndi,
     3      ipars,nqorder,ntrimax,rfac,sints_n,ifmetric,rn1,n2)
...
c
        ...
        if(iptype(ipatch).eq.1) 
     1     call ctriaints_wnodes_c(ntest0,norder,npols,
     1     srccoefs_c(1,istart),ndtarg,ntarg_f,targ_far,
     2      itargptr,ntarg_f,norder,npols,fker,ndd,dpars,ndz,zpars,
     3      ndi,ipars,npts_f,qnodes,qwts,sints_f)
...
ccccc!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          if(ipatch.eq.-jpatch) then
            call zget_ggq_self_quad_pt_c(ipv,norder,npols,
     1      uvs_targ(1,jtarg),umatr,srccoefs_c(1,istart),ndtarg,
     2      targvals_c(1,jtarg),irad,fker,ndd,dpars,ndz,zpars,ndi,
     3      ipars,wnear(iqstart+1))
          endif
...
c
c
      subroutine zget_ggq_self_quad_pt_c(ipv,norder,npols,uvs,umat,
     1   srccoefs,ndtarg,targ,irad,fker,ndd,dpars,ndz,zpars,ndi,
     2   ipars,zquad)
c
c
...
c
      ...
      implicit real *8(a-h,o-z)
      integer, intent(in) :: norder,npols,ndtarg
      complex *16, intent(in) :: srccoefs(9,npols)
      real *8, intent(in) :: targ(ndtarg)
      real *8, intent(in) :: uvs(2),umat(npols,npols)
      integer, intent(in) :: ndi,ndd,ndz
      integer, intent(in) :: ipars(ndi)
      ...
      complex *16 srcvals(9)
      real *8, allocatable :: sigvalstmp(:)
      complex *16, allocatable :: srctmp(:,:)
      complex *16, allocatable :: qwts(:)
      real *8, allocatable ::  sigvals(:,:)
      complex *16, allocatable :: sigvals_c(:,:)
      complex *16, allocatable :: xs(:),ys(:),ws(:)
      complex *16 uv(2)
      real *8 verts(2,3)
      real *8 alpha,beta
      complex *16 fval
...
c
      ...
      do ii=1,npols
      csigvalstmp(ii) = sigvalstmp(ii)
      enddo

      alpha = 1.0d0
      beta = 0.0d0
      call zgemv_guru('n',9,npols,alpha,srccoefs,9,csigvalstmp,1,beta,
     1      srcvals,1)
      ...
      ns = 0
      if(ipv.eq.0) then
        call self_quadrature_new_c(irad,verts,uvs(1),
     1    uvs(2),srcvals(4),ns,xs,ys,ws)
      endif

      if(ipv.eq.1) then
        call pv_self_quadrature_new_c(irad,verts,uvs(1),
     1    uvs(2),srcvals(4),ns,xs,ys,ws)
      endif
     
      allocate(srctmp(12,ns),qwts(ns))
      allocate(sigvals(npols,ns))
      allocate(sigvals_c(npols,ns))
      ...
      do ii=1,ns
      do jj=1,npols
      sigvals_c(jj,ii) = sigvals(jj,ii)
      enddo
      enddo

      call zgemm_guru(transa,transb,9,ns,npols,alpha,
     1      srccoefs,lda,sigvals_c,ldb,beta,srctmp,ldc)


c
      do i=1,ns
        call cross_prod3d_c(srctmp(4,i),srctmp(7,i),srctmp(10,i))
        rr = sqrt(srctmp(10,i)**2+srctmp(11,i)**2+srctmp(12,i)**2)
        qwts(i) = rr*ws(i)
        srctmp(10,i) = srctmp(10,i)/rr
        srctmp(11,i) = srctmp(11,i)/rr
