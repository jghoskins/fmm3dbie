...
c
c
        subroutine self_quadrature_new_c(irad, verts0, x0, y0, dr,
     1    nquad, xs, ys, whts)
        implicit double precision (a-h,o-z)
        dimension verts0(2,3)
        complex *16 a(2,2),ainv(2,2),xs(*),ys(*),whts(*),dr(3,2)
        dimension b(3,2)
        complex *16 xx,yy,zz,verts(2,3),det,s,t,wht,u,v
c
c       Return a quadrature formula for evaluating integrals of the
c       form (1).
c
...
c       conformal.  Also, compute the inverse of A.
c
        call find_conf_map_c(dr,a,ainv)
c
c
c       Apply the inverse of A to the vertices of the triangle T in
c       order to find the vertices of the triangle T_0.
...
c       Apply the mapping A to the quadrature formula.
c
        det = sqrt((a(1,1)*a(2,2)-a(1,2)*a(2,1))**2)
c
        do i=1,nquad
          s   = xs(i)
          t   = ys(i)
...
c
c
        subroutine pv_self_quadrature_new_c(irad,verts0,x0,y0,dr,
     1       nquad,xs,ys,whts)
        implicit double precision (a-h,o-z)
        integer irad
        complex *16 verts(2,3),dr(3,2),a(2,2),ainv(2,2)
        real *8 verts0(2,3)
        complex *16 xs(*),ys(*),whts(1),b(3,2),xx,yy,
     1      u,v,s,t,wht,det
c
c       Return a quadrature formula for evaluating integrals of the
c       form (1).
...
c
c
        subroutine find_conf_map_c(a,b,binv)
        implicit double precision (a-h,o-z)
        complex *16 a(3,2),b(2,2),binv(2,2),q(3,2),
     1      sum1,dip,sum2,det
c
c       Given a (3,2) matrix A, return a (2,2) matrix B such that
c       A*B has orthonormal columns.  Also, return the inverse
